...
from continuous_exploration import ContinuousExploration
...
class CausalDataGenerator:
    ...
    def _generate_single_episode(self, assignment):
        ...
        try:
            ...
            explorer = ContinuousExploration(goal_bias=0.7, exploration_noise=0.3)
            
            # Convert causal state dictionary to CausalState object
            causal_state_obj = CausalState(
                time_hour=causal_state['time_hour'],
                day_week=causal_state['day_week'],
                weather=WeatherType(causal_state['weather']),
                event=EventType(causal_state['event']),
                crowd_density=causal_state['crowd_density'] / 5.0  # Normalize to [0,1]
            )

            # Reset with specified causal state
            goal_types = ['library', 'gym', 'cafeteria', 'academic', 'random']
            goal_type = random.choice(goal_types)

            observation, info = env.reset(options={
                'causal_state': causal_state_obj,
                'goal_type': goal_type
            })
            ...
            while t < time_steps:
                ...
                # Extract causal encoding from observation for compatibility
                causal_encoding = observation[7:12]  # Last 5 elements are causal factors
                causal_sequence.append(causal_encoding.copy())
                ...
                info_sequence.append({
                    ...
                    'velocity': observation[4:6].copy()
                })

                # Select action using continuous exploration
